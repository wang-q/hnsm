...
EXAMPLES:
    ...
    # Custom size
    hnsm synt view blocks.tsv -o plot.svg --width 1200 --height 800
"#,
        )
        .arg(
            Arg::new("infile")
...
pub fn execute(matches: &clap::ArgMatches) -> anyhow::Result<()> {
    ...
    let height = *matches.get_one::<u32>("height").unwrap() as f64;
    ...
    writeln!(writer, r#"<svg width="{}" height="{}" xmlns="http://www.w3.org/2000/svg">"#, width, height)?;
    ...
    // Draw Tracks
    for (i, g_name) in genome_order.iter().enumerate() {
        let y = margin_y + i as f64 * track_height + track_height / 2.0;
        
        // Draw Label
        if !no_label {
            writeln!(writer, r#"<text x="{}" y="{}" dy="-10" font-weight="bold">{}</text>"#, margin_x, y, g_name)?;
        }
        ...
        if let Some(chroms) = genome_chroms.get(g_name) {
            ...
            for chrom in sorted_chroms {
                let x = *chrom_offsets.get(&chrom).unwrap();
                let len = *chrom_lengths.get(&chrom).unwrap_or(&0) as f64;
                let w = len * scale_x;
                ...
                if !no_label {
                    writeln!(writer, r##"<text x="{:.1}" y="{:.1}" dy="25" font-size="10" fill="#666">{}</text>"##, x + w/2.0, y, chrom)?;
                }
            }
        }
    }
    ...
    for (block_idx, block) in blocks.iter().enumerate() {
        ...
        let g1 = seq_to_genome.get(&r1.seq_name);
        let g2 = seq_to_genome.get(&r2.seq_name);
        ...
        // Get Y coordinates
        let y1_idx = genome_order.iter().position(|g| g == g1).unwrap();
        let y2_idx = genome_order.iter().position(|g| g == g2).unwrap();
        
        if y1_idx == y2_idx { continue; } // Intra-genome not supported well yet
        
        let y1 = margin_y + y1_idx as f64 * track_height + track_height / 2.0 + 10.0; // Bottom of bar
        let y2 = margin_y + y2_idx as f64 * track_height + track_height / 2.0; // Top of bar
        
        // Get X coordinates
        let x1_off = *chrom_offsets.get(&r1.seq_name).unwrap();
        let x2_off = *chrom_offsets.get(&r2.seq_name).unwrap();
        
        let x1_start = x1_off + r1.start as f64 * scale_x;
        let x1_end = x1_off + r1.end as f64 * scale_x;
        
        // Handle strand for r2
        let (x2_start, x2_end) = if r1.strand == r2.strand {
            (x2_off + r2.start as f64 * scale_x, x2_off + r2.end as f64 * scale_x)
        } else {
            // Invert visualization for inverted alignment?
            // Usually we draw a "twist".
            (x2_off + r2.end as f64 * scale_x, x2_off + r2.start as f64 * scale_x)
        };
        ...
        // Bezier Path
        // M x1_start y1
        // C cp1x cp1y, cp2x cp2y, x2_start y2
        // L x2_end y2
        // C cp3x cp3y, cp4x cp4y, x1_end y1
        // Z
