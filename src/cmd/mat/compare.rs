...
Examples:
    ...
    hnsm mat compare matrix1.phy matrix2.phy --method pearson

    # Compare using multiple methods
    hnsm mat compare matrix1.phy matrix2.phy --method pearson,cosine,jaccard
"###,
        )
        .arg(
            Arg::new("matrix1")
...
pub fn execute(args: &ArgMatches) -> anyhow::Result<()> {
    ...
    // Load matrices
    let matrix1 = hnsm::NamedMatrix::from_relaxed_phylip(matrix1_file);
    let matrix2 = hnsm::NamedMatrix::from_relaxed_phylip(matrix2_file);
    ...
    for method in methods.split(',') {
        let result = match method {
            "pearson" => hnsm::pearson_correlation(&values1, &values2),
            "spearman" => spearman_correlation(&values1, &values2),
            "mae" => mean_absolute_error(&values1, &values2),
            "cosine" => hnsm::cosine_similarity(&values1, &values2),
            "jaccard" => hnsm::weighted_jaccard_similarity(&values1, &values2),
            "euclid" => hnsm::euclidean_distance(&values1, &values2),
            _ => unreachable!(),
        };
...
fn spearman_correlation(x: &[f32], y: &[f32]) -> f32 {
    ...
    hnsm::pearson_correlation(&x_ranks, &y_ranks)
}
